...
// 1
const divByThree = pokémon.filter((pokemon) => pokemon.id % 3 === 0);
...
// 5
const greaterThanNinetyNine = pokémon.filter((pokemon) => pokemon.id > 99);
console.log(greaterThanNinetyNine);

// 6
const namesOfPoison = pokémon
  .filter((pokemon) => pokemon.types.includes("poison"))
  .map((pokemon) => pokemon.name);
console.log(namesOfPoison);

// 7
const secondTypeFlying = pokémon.filter(
  (pokemon) => pokemon.types.includes("flying") && pokemon.types.length === 2
);
console.log(secondTypeFlying);

// 8
const numOfNormalTypes = pokémon.filter((pokemon) =>
  pokemon.types.includes("normal")
).length;
console.log(numOfNormalTypes);
